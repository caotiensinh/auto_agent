...
log(){ printf '\033[1;34m[UI-V060]\033[0m %s\n' "$*"; }
ok(){ printf '\033[1;32m[UI-V060:OK]\033[0m %s\n' "$*"; }
die(){ printf '\033[1;31m[UI-V060:FAIL]\033[0m %s\n' "$*" >&2; exit 1; }
...
for marker in 'id="messages"' 'id="prompt"' 'id="mode"' 'id="send"' '__CENTER_PUBLIC_HOST__' 'AUTO AGENT' 'composer-wrap'; do
...
for line in lines:
    if line.startswith("Description=auto_agent Unified Control Center"):
        out.append("Description=auto_agent Unified Control Center v0.6.0 UI")
...
ok "Hermes-inspired full-height sidebar installed"
ok "ChatGPT/Claude-style bottom composer installed"
ok "Responsive mobile navigation installed"
ok "Existing SSO/auth/backend logic preserved"
printf '\n============================================================\n'
printf 'AUTO_AGENT UI v0.6.0 READY\n'
printf 'Layout   : full-height left navigation\n'
