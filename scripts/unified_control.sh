...
Environment=AUTO_AGENT_HERMES_PUBLIC_PORT=$HERMES_PORT
Environment=AUTO_AGENT_OPENCLAW_PUBLIC_PORT=$OPENCLAW_PORT
Environment=AUTO_AGENT_HERMES_BIN=$HERMES
Environment=AUTO_AGENT_OPENCLAW_BIN=$OPENCLAW
Environment=AUTO_AGENT_OPENCLAW_URL=http://127.0.0.1:$OPENCLAW_PORT
...
server {
  listen ${LAPTOP_IP}:${OPENCLAW_PORT};
  server_name _;
  location = /_auth {
    internal;
    proxy_pass http://127.0.0.1:${CENTER_INTERNAL_PORT}/auth/check;
  ...
  location / {
    ...
    proxy_set_header X-Forwarded-For \$remote_addr;
    proxy_set_header X-Forwarded-Proto \$scheme;
    proxy_set_header X-Forwarded-User auto-agent-admin;
    proxy_set_header Upgrade \$http_upgrade;
    proxy_set_header Connection \$auto_agent_upgrade;
...
ok "Nginx single-login proxy configured"

if command -v ufw >/dev/null 2>&1 && sudo ufw status 2>/dev/null | grep -q '^Status: active'; then
  for port in "$CENTER_PORT" "$HERMES_PORT" "$OPENCLAW_PORT"; do
    sudo ufw allow from "$LAN_CIDR" to "$LAPTOP_IP" port "$port" proto tcp >/dev/null
  done
  ok "UFW permits control ports only from $LAN_CIDR"
else
  warn "UFW inactive; login is enforced but subnet scope is not enforced by host firewall"
fi
...
hcode="$(curl -sS -o /dev/null -w '%{http_code}' "http://${LAPTOP_IP}:${HERMES_PORT}/" || true)"
ocode="$(curl -sS -o /dev/null -w '%{http_code}' "http://${LAPTOP_IP}:${OPENCLAW_PORT}/" || true)"
...
printf 'Hermes UI      : menu inside Control Center\n'
printf 'OpenClaw UI    : menu inside Control Center\n'
